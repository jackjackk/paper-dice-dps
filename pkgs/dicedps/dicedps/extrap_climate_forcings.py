...
def extrap_forcings(forcpath, bplot=False):
    ...
    dffinal = pd.concat([df2extrap.sort_index(axis=1),
                         pd.DataFrame(yfut).sort_index(axis=1)], axis=0,
                        keys=['data','pred'], names=['type','year']).mean(axis=0,level='year')
    print(f'forcpath = {forcpath}')
    df2write = dffinal.stack('source').swaplevel().sort_index(axis=0)
    print(f'dataframe to write:\n',df2write)
    df2write.to_csv(forcpath)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extrapolate forcings')
    parser.add_argument('-o', '--output', dest='output', type=str, action='store',
                        default=indicedps('data','forcings.csv'), help='Output csv file path')
