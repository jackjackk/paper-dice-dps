...
def eps_sort_solutions(tables, epsilons=None):
    ...
    for table in tqdm(tables):
        for objectives, row in tqdm(table):
...
def withobjectives(annotatedrows, oindices):
    ...
    if oindices is not None:
        for i, (row, annot) in enumerate(annotatedrows):
            objectives = []
            try:
                for oo in oindices:
...
def cli(args):
    ...
    if args.print_only_objectives and args.objectives is not None:
        if args.contribution and args.reverse_column_indices:
            for ii, obj in enumerate(args.objectives):
                args.objectives[ii] = obj-1
            args.objectives.append(-1)
        for row in tagalongs:
