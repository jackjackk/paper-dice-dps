...
option_list = list(
  make_option(c("-r", "--rdsfile"), type="character", default="brick_mcmc_fgiss_TgissOgour_scauchy_t18802011_z18801900_o4_h50_n10000000.rds", help="rds file"),
  make_option(c("-d", "--diag"), type="character", default="hd", help="Heidelberger and Welch (hd), Gelman and Rubin (gd)"),
...
if (opt$diag == "hd") {
  ...
  ret = gelman.diag(mcmc.chains, autoburnin = FALSE)
  print(ret)
  saveRDS(ret, file=paste0(basename, "-gd.rds"))
} else if (opt$diag == "gplot") {
  png(paste0(basename, '-gplot.png'), width=1920, height=1080)
  par(mfrow=c(4,7))
  gelman.plot(mcmc.chains, auto.layout = FALSE)
  dev.off()
} else if (opt$diag == "acf") {
  png(paste0(basename, '-acf.png'), width=1920, height=1080)
  par(mfrow=c(2,2))
  for (m in 1:nnode.mcmc) {
    acf(mcmc.chains[[m]])
  }
  ...
  library(batchmeans)
  selist <- vector("list", nnode.mcmc)
  for (m in 1:nnode.mcmc) {
    selist[[m]] = bmmat(mcmc.chains[[m]])
    print(selist[[m]])
  }
  saveRDS(selist, file=paste0(basename, "-se.rds"))
} else if (opt$diag == "tplot") {
  png(paste0(basename, '-tplot.png'), width=1920, height=1080)
  library(mcmcplots)
  mcmcplots::traplot(mcmc.chains)
  dev.off()
} else if (opt$diag == "rds") {
  saveRDS(mcmc.chains, file=paste0(basename, "-chains.rds"))
} else {
  png(paste0(basename, "-", opt$diag, ".png"), width=1920, height=1080)
  library(mcmcplots)
  do.call(opt$diag, list(mcmcout=mcmc.chains))
  dev.off() 
}
