...
option_list = list(
  ...
  make_option(c("-n", "--niter"), type="integer", default=1000, help="number of iterations"),
  make_option(c("-N", "--nnode"), type="integer", default=1, help="number of chains"),
  ...
  make_option(c("-s", "--sprior"), type="character", default="cauchy", help="prior for climate sensitivity in {cauchy, paleosens, chylek, unif}"),
  make_option(c("-x", "--save"), type="character", default="rds", help="save either via 'workspace' or 'rds'"),
  make_option(c("-o", "--outdir"), type="character", default="../../../data", help="output directory, no trailing slash"),
...
basefile = sprintf("brick_mcmc_f%s_T%sO%s_s%s_t%d%d_z%d%d_o%d_h%d_n%d",
                     opt$forcing,
                     opt$temp,
                     opt$oheat,
                     opt$sprior,
...
filename.saveprogress <- paste0(basefile,'.RData')
...
if (opt$save == "rds") {
saveRDS(amcmc.par1, file=paste0(opt$outdir,"/",rdsfile))
}
} 
...
## Diagnostic plots
if(l.doplots) {
png(paste0(basefile, '-iter.png'), width=1920, height=1080)
## Check #1: History plots
par(mfrow=c(5,5))
iter.subsample = seq(5e4,10e6,10e3) 
for (pp in 1:length(parnames)) {
  plot(iter.subsample, chain1$samples[iter.subsample,pp], type="l", ylab=parnames[pp], xlab="Number of Runs", main="")
}
dev.off()
}
...
niter.test = seq(from=50000, to=niter.mcmc, by=100000)
niter.test = seq(from=niter.mcmc, to=niter.mcmc, by=100000)
...
if(nnode.mcmc == 1) {
  ...
  for (i in 1:length(niter.test)) {
      print(paste0("test ",i,"/",length(niter.test)))
...
filename.parameters = paste0(basename,'.nc')
